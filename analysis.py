...
from dietData.analysis.neighbors import Fitting
...
movement_report_path = os.path.join(par_dir, "reports", "movement-report_reduction-%s_scaler-%s_continent-%s_cut-%s-%s_zanan-%s.csv" % param_vals)
fitting_report_path = os.path.join(par_dir, "reports", "fitting-report_reduction-%s_scaler-%s_continent-%s_cut-%s-%s_zanan-%s.csv" % param_vals)
...
fitting_analyzer = Fitting(dataset_2d_save_path)
fitting_analyzer.analyze_fitting(fitting_report_path)
